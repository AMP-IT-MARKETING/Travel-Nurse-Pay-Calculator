...
import { PayPackage, CalculationResult, OTModel, Job } from './types';
...
const TabButton: React.FC<{
  ...
  <button
    ...
    className={`flex items-center space-x-2 px-4 py-3 font-semibold text-sm rounded-t-lg transition-colors focus:outline-none focus:ring-2 focus:ring-[#00AAA1] ${
      isActive
        ? 'bg-white text-[#00AAA1]'
...
const App: React.FC = () => {
  ...
  const [activeView, setActiveView] = useState<'jobs' | 'calculator'>('jobs');
  ...
  const handleSelectJob = useCallback((job: Job) => {
    setInputs(prev => ({
        ...
        contractWeeks: parseInt(job.contractWeeks, 10) || 13,
        weeklyHours: parseInt(job.weeklyHours, 10) || 0,
        taxableBaseRate: job.payPackage.taxableRate || 0,
        // Assign the entire stipend from the job to housing. The user can split it if needed.
        stipendHousingWeekly: job.weeklyStipend > 0 ? job.weeklyStipend : 0,
        stipendMieWeekly: 0,
        hasTaxHome: prev.hasTaxHome, // Persist tax home choice
    }));
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-[#3E4E56]">
      ...
      <main className="container mx-auto p-4 sm:p-6 lg:p-8">
        <div className="flex space-x-2 border-b mb-6" role="tablist">
          <TabButton 
            label="Job Board" 
